...
export default function VerifyEmail({ route, navigation }) {
	...
	useEffect(() => {
		...
		inputRef.forEach(
			(value, index) => index == indexRef.current && value.current.focus()
		);
		indexRef.current++;
	}, [inputRefOne, inputRefTwo, inputRefThree, inputRefFour]);
			...
			>
				<View style={styles.textCode}>
					<TextInput
						...
						ref={inputRefOne}
						value={inputRefOne.current.value}
						onChangeText={(text) =>
							(inputRefOne.current.value = text)
						}
					/>
				</View>
				<View style={styles.textCode}>
